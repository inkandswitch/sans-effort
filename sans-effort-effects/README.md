...
> A standard library of common effect traits for `sans-effort` routines
...
A trait returns `Result` exactly when its effect can fail for reasons outside the routine. A fallible effect's request names its real answer (`type Reply = Result<String, ReadLineError>`), which crosses as `bytes` in `sans-effort-core`'s encoding; bytes that do not decode as it are refused before the routine sees them.
