...
| `code`, `Status`, `Error`                 | The status and error codes as they cross the ABI (`ABI_VERSION` among them), and their Rust forms — including `Error::WrongKind`, a reply of the wrong kind for its id, which leaves the request outstanding                                                                        |
